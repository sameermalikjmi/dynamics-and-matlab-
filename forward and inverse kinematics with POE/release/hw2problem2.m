...
clear, clc, close all
addpath('utils');
% issue with when all q ranges from pi/6 and n tests is 20 as .2708 is q
% values for all.
plotOn = false;
...
% Put all the axes into a 6xn matrix S, where n is the number of joints
    ...
    S2 =  transpose([-1 0 0 0 -0.3 0]);
   % S3= transpose([1 0 0 0.3 0 0]);
   
   S3= transpose([-1 0 0 0 -0.3 0.3]);
 S4= transpose([0 0 1 0.6 0 0]);
  S5= transpose([-1 0 0 0 -0.3 0.6]);
   S6= transpose([0 1 0 -0.3 0 0]);
    S= [S1 S2 S3 S4 S5 S6];
